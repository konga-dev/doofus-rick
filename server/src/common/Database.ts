...
import type { Db, DeleteResult, Filter, InsertOneResult, OptionalUnlessRequiredId, UpdateFilter, UpdateResult } from 'mongodb'
...
class Database {
	...
	public async connect(databaseName: string): Promise<void> {
		...
		try {
			...
			// biome-ignore lint: jo keine ahnung oida
		} catch (error: any) {
